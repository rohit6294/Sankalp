...
const linkToAdd = `\n    <a href="counseling.html" class="sidebar-link"><i class="fas fa-comments" style="width:18px"></i> 1-on-1 Counseling</a>`;
const activeLinkToAdd = `\n    <a href="counseling.html" class="sidebar-link active"><i class="fas fa-comments" style="width:18px"></i> 1-on-1 Counseling</a>`;
...
for (const file of files) {
  if (file === 'counseling.html') continue; // already has it
  const filePath = path.join(studentDir, file);
  let content = fs.readFileSync(filePath, 'utf8');

  if (content.includes('href="counseling.html" class="sidebar-link')) {
    continue;
  }
